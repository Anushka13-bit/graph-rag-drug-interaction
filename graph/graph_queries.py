...
MATCH path = (d1:Drug {name: $drug1})-[:INTERACTS_WITH*1..3]-(d2:Drug {name: $drug2})
RETURN [node IN nodes(path) | coalesce(node.name, node.description)] AS drug_chain,
       [rel IN relationships(path) | rel.type] AS interaction_types,
       length(path) AS hops
ORDER BY hops ASC
LIMIT $limit
"""
...
MATCH (d1:Drug {name: $drug1})-[:INTERACTS_WITH]->(shared:Drug)<-[:INTERACTS_WITH]-(d2:Drug {name: $drug2})
